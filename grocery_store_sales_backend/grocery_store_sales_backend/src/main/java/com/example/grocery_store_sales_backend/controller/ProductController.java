...
public class ProductController {
    ...
    ,@RequestParam("search")String search,@RequestParam("searchStatus")String searchStatus){
        Pageable pageable= PageRequest.of(Integer.parseInt(page),10);
        if(productService.getProducts(pageable,Integer.valueOf(type),search,  Integer.valueOf(searchStatus) ).isEmpty()){
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
        }else {
            return new ResponseEntity<>(productService.getProducts(pageable,Integer.valueOf(type),search,  Integer.valueOf(searchStatus)),HttpStatus.OK);
        }
    }
    @GetMapping("/type")
